...
<div class="wrap">
  ...
  <section id="remake-analysis" class="page">
    ...
    <div class="card"><h3 class="section-title">By Culprit</h3><table><thead><tr><th>Culprit</th><th>Subcategory</th><th class="right">Orders</th><th class="right">QTY</th><th class="right">% of subtotal</th></tr></thead><tbody id="remakeCulpritBody"></tbody></table></div>
...
function renderForensics(factoryFilter, periodKey) {{
  ...
  function groupHtml(g, kind) {{ const subtotal=Object.keys(g).reduce(function(sum,k) {{ return sum + Number(g[k].qty||0); }},0); const orderTotal=Object.keys(g).reduce(function(sum,k) {{ return sum + Number(g[k].orders||0); }},0); return Object.keys(g).sort(function(a,b) {{ return (g[b].qty||0)-(g[a].qty||0) || a.localeCompare(b); }}).map(function(k) {{ const item=g[k], share=subtotal ? (item.qty/subtotal*100).toFixed(1)+'%' : '0.0%', filterValue=kind === 'culprit' ? item.culprit : k, active=(kind === 'category' ? remakeAnalysisCategoryFilter : remakeAnalysisCulpritFilter) === filterValue; return '<tr class="remake-filter-row'+(active ? ' active' : '')+'" data-filter-kind="'+kind+'" data-filter-value="'+escapeAttr(filterValue)+'" tabindex="0" role="button" aria-pressed="'+(active ? 'true' : 'false')+'"><td>'+esc(kind === 'culprit' ? item.culprit : k)+'</td>'+(kind === 'culprit' ? '<td>'+esc(item.subcategory||'—')+'</td>' : '')+'<td class="right">'+item.orders.toLocaleString()+'</td><td class="right">'+item.qty.toLocaleString()+'</td><td class="right">'+share+'</td></tr>'; }}).join('') + '<tr class="total-row"><td'+(kind === 'category' ? '' : ' colspan="2"')+'><strong>Subtotal</strong></td><td class="right"><strong>'+orderTotal.toLocaleString()+'</strong></td><td class="right"><strong>'+subtotal.toLocaleString()+'</strong></td><td class="right"><strong>100.0%</strong></td></tr>'; }}
