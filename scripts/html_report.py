...
function renderForensics(factoryFilter) {{
  ...
  function groupHtml(g) {{ return Object.keys(g).sort(function(a,b) {{ return (g[b].qty||0)-(g[a].qty||0) || a.localeCompare(b); }}).map(function(k) {{ return '<tr><td>'+esc(k)+'</td><td class="right">'+g[k].orders.toLocaleString()+'</td><td class="right">'+g[k].qty.toLocaleString()+'</td></tr>'; }}).join(''); }}
  ...
  document.getElementById('qcErrorTypeBody').innerHTML = groupHtml(et); document.getElementById('qcSubcategoryBody').innerHTML = groupHtml(escat); document.getElementById('qcPreventionBody').innerHTML = Object.keys(pa).sort(function(a,b) {{ return pa[b]-pa[a] || a.localeCompare(b); }}).map(function(k) {{ return '<tr><td>'+esc(k)+'</td><td class="right">'+pa[k].toLocaleString()+'</td></tr>'; }}).join('');
}}
