...
function showFactorySharePage(slug) {{
  ...
  function renderFactoryAnalysis(selectedList) {{
    ...
    const rem=(typeof REMAKES!=='undefined'?REMAKES:[]).filter(function(r) {{ return matchFactory(r) && matchPeriod(r) && selectedOrderSet.has(String(r.order||'')); }});
