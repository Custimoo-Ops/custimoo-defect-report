...
<div class="wrap">
  ...
  <section id="remake-analysis" class="page">
    ...
    <div class="card"><h3 class="section-title">Unreviewed Remake Orders</h3><div style="overflow:auto;max-height:55vh"><table><thead><tr><th>Order</th><th>Customer</th><th>QTY</th><th>% of subtotal</th><th>Factory</th><th>Admin</th><th>Month</th></tr></thead><tbody id="remakeUnreviewedBody"></tbody></table></div></div>
    ...
    <div class="card"><h3 class="section-title">By Culprit</h3><table><thead><tr><th>Culprit</th><th class="right">Orders</th><th class="right">QTY</th><th class="right">% of subtotal</th></tr></thead><tbody id="remakeCulpritBody"></tbody></table></div>
  </section>
...
function renderForensics(factoryFilter) {{
  ...
  const unreviewedSubtotal = remUn.reduce(function(sum,r) {{ return sum+Number(r.qty||0); }},0);
  ...
  document.getElementById('remakeUnreviewedBody').innerHTML = remUn.slice().sort(function(a,b) {{ return Number(b.qty||0)-Number(a.qty||0); }}).map(function(r) {{ return '<tr><td>#'+esc(remakeOrderKey(r))+'</td><td>'+esc(r.customer||'(unknown)')+'</td><td class="right">'+(Number(r.qty)||0).toLocaleString()+'</td><td class="right">'+(unreviewedSubtotal ? (Number(r.qty||0)/unreviewedSubtotal*100).toFixed(1)+'%' : '0.0%')+'</td><td>'+esc(r.factory||'')+'</td><td>'+esc(r.admin||'')+'</td><td>'+esc(r.month||'')+'</td></tr>'; }}).join('') || '<tr><td colspan="7">All remake orders have forensic notes.</td></tr>';
