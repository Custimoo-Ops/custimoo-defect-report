...
function showFactorySharePage(slug) {{
  ...
  document.getElementById('factoryShareKpis').innerHTML = [['Total Orders',periodFactory.orders||0],['Total Order QTY',periodFactory.volume||0],['Remake Orders',periodFactory.remake_orders||0],['Remake QTY',periodFactory.remake_qty||0],['Qarma Orders Checked',periodQ.orders_checked||0],['Qarma Error Orders',periodQ.rejected_orders||0],['Orders with Reinspection',periodQ.orders_with_reinspection||0,'reinspection']].map(function(x) {{ return '<div class="card metric' + (x[2] ? ' factory-filter-card' : '') + '"' + (x[2] ? ' data-filter="'+x[2]+'" title="Click to show orders with reinspection"' : '') + '><div class="label">'+x[0]+'</div><div class="value">'+Number(x[1]||0).toLocaleString()+'</div></div>'; }}).join('');
  ...
  document.querySelectorAll('.factory-filter-card').forEach(function(card) {{ card.style.cursor='pointer'; card.addEventListener('click', function() {{ const subset=orders.filter(function(r) {{ return r.qarma_reinspected; }}); document.getElementById('factoryShareOrders').innerHTML = subset.map(function(r) {{ const qlink=/^https?:\\/\\//i.test(r.qarma_report_link||'') ? '<a href="'+escapeAttr(r.qarma_report_link)+'" target="_blank" rel="noopener">Open report</a>' : '—'; return '<tr><td>#'+esc(r.order)+'</td><td>'+esc(r.completed_date||'')+'</td><td>'+esc(r.customer||'')+'</td><td class="right">'+Number(r.qty||0).toLocaleString()+'</td><td>'+ (r.qarma_checked?'Yes':'No') +'</td><td>'+ (r.qarma_rejected?'Yes':'No') +'</td><td>Yes</td><td>'+esc(r.qarma_comment||'—')+'</td><td>'+qlink+'</td><td>'+ (r.remake?'Yes':'No') +'</td></tr>'; }}).join('') || '<tr><td colspan="10">No re-inspected orders in the selected period.</td></tr>'; }}); }});
