...
function showFactorySharePage(slug) {{
  ...
  function renderRemakeSelectedRows(list) {{
    ...
    document.getElementById('factoryShareOrders').innerHTML=list.map(function(r) {{ const a=byOrder.get(String(r.order||'').replace(/^#/,'').trim())||{{}}; return '<tr><td>#'+esc(r.order)+'</td><td>'+esc(r.customer_ref||'—')+'</td><td>'+dateOnly(r.created_date)+'</td><td>'+dateOnly(r.actual_shipping_date)+'</td><td>'+esc(r.customer||'')+'</td><td class="right">'+Number(r.qty||0).toLocaleString()+'</td><td>'+esc(a.category||'Uncategorized')+'</td><td>'+esc(a.culprit||'Culprit not set')+'</td><td>'+esc(a.culprit_subcategory||'—')+'</td><td>'+esc(a.verification_status||'—')+'</td><td>'+esc(a.comment||'—')+'</td></tr>'; }}).join('') || '<tr><td colspan="11">No remake orders in the selected period.</td></tr>';
  }}
