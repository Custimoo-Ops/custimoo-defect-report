...
HAVING bool_and(oi.status::text <> 'order_cancel')
""")
...
for _qr in load_qarma_rows():
    ...
    if str(_qr.get('Reinspection of') or '').strip():
        _qd['reinspected'] = True
        _rid = str(_qr.get('Report inspection id') or _qr.get('Inspection id') or _qr.get('Link to report') or '').strip()
        if _rid: _qd['reinspection_ids'].add(_rid)
...
for _order,_created,_date,_actual_shipping_date,_shipping_date,_qty,_factories,_customer,_customer_ref,_otype in _factory_share_cur.fetchall():
    ...
    _row = {'order': str(_order), 'created_date': str(_created)[:19] if _created else '', 'completed_date': str(_date)[:19] if _date else '', 'actual_shipping_date': str(_actual_shipping_date)[:19] if _actual_shipping_date else '', 'backend_shipping_date': str(_shipping_date)[:19] if _shipping_date else '', 'qty': int(_qty or 0), 'customer': _customer or '(unknown)', 'customer_ref': str(_customer_ref or ''), 'qarma_checked': bool(_q), 'qarma_error': bool(_q.get('defects', 0)), 'qarma_rejected': bool(_qd.get('rejected')), 'qarma_reinspected': bool(_qd.get('reinspected')), 'qarma_reinspection_count': len(_qd.get('reinspection_ids', set())), 'qarma_months': sorted(_qd.get('months', set())), 'qarma_comment': ' · '.join(_qd.get('comments', [])), 'qarma_report_link': (_qd.get('links') or [''])[0], 'remake': str(_order) in REMAKE_ORDERS}
    for _factory in str(_factories or '(unknown)').split(','):
        ...
        _fr['qarma_checked_factory'] = bool(_factory_norm in _qd.get('factories', set()))
        ...
        _fr['qarma_reinspection_count_factory'] = len(_qd.get('reinspection_ids', set())) if _factory_norm in _qd.get('factories', set()) else 0
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .remake-mgmt-scroll .remake-table {{ min-width: 2660px; table-layout: fixed; }}
  ...
  .remake-mgmt-scroll .remake-table th:nth-child(15) {{ width: 520px; }}
  .remake-mgmt-scroll .remake-table th, .remake-mgmt-scroll .remake-table td {{ vertical-align: top; }}
  .remake-mgmt-scroll .remake-table td:nth-child(15) {{ white-space: normal; }}
...
<div class="wrap">
  ...
  <section id="remake-analysis" class="page">
    <div class="card"><div class="section-head"><h3 class="section-title">Remake Forensics — Categories and Culprits</h3><label class="muted">Factory <select id="remakeAnalysisFactoryFilter" class="filter-select"><option value="">All factories</option></select></label><label class="muted">Period <select id="remakeAnalysisPeriodFilter" class="filter-select"><option value="all">All</option><option value="ytd" selected>YTD 2026</option><option value="last_3">Last 3 months</option><option value="last_6">Last 6 months</option><option value="last_month">Last month</option><option value="mtd">MTD</option><option value="quarter">Quarter</option></select></label><label class="muted">Responsibility <select id="remakeAnalysisResponsibilityFilter" class="filter-select"><option value="">All</option><option value="Factory">Factory</option><option value="Custimoo">Custimoo</option><option value="Unassigned">Unassigned</option></select></label><label class="muted"><input type="checkbox" id="remakeAnalysisExcludeForceMajour"> Exclude Force Majour</label></div><div class="hint">Reviewed means Category and Culprit are assigned, any required Custimoo subcategory is set, and the team has added a comment or verification status. A Category alone is not complete.</div><div id="remakeAnalysisKpis" class="exec-grid"></div></div>
    ...
    <div class="card"><h3 class="section-title">Largest Unreviewed Remake Orders</h3><div class="hint">Outstanding orders sorted by QTY, largest first. Percentages use the displayed top-25 subtotal.</div><div style="overflow:auto;max-height:55vh"><table><thead><tr><th>Order</th><th>Customer</th><th>QTY</th><th>% of subtotal — orders</th><th>% of subtotal — QTY</th><th>Factory</th><th>Admin</th><th>Month</th></tr></thead><tbody id="remakeLargeUnreviewedBody"></tbody></table></div></div>
    <div class="card"><h3 class="section-title">By Category</h3><table><thead><tr><th>Category</th><th class="right">Orders</th><th class="right">% of subtotal — orders</th><th class="right">QTY</th><th class="right">% of subtotal — QTY</th></tr></thead><tbody id="remakeCategoryBody"></tbody></table></div>
    <div class="card"><h3 class="section-title">By Culprit</h3><table><thead><tr><th>Culprit</th><th>Subcategory</th><th class="right">Orders</th><th class="right">% of subtotal — orders</th><th class="right">QTY</th><th class="right">% of subtotal — QTY</th></tr></thead><tbody id="remakeCulpritBody"></tbody></table></div>
  ...
  <section id="qc-analysis" class="page">
    ...
    <div class="card"><h3 class="section-title">By Error Type</h3><table><thead><tr><th>Error Type</th><th class="right">Orders</th><th class="right">Defect QTY</th><th class="right">% of subtotal</th></tr></thead><tbody id="qcErrorTypeBody"></tbody></table></div>
    <div class="card"><h3 class="section-title">By Error Subcategory</h3><table><thead><tr><th>Error Subcategory</th><th class="right">Orders</th><th class="right">Defect QTY</th><th class="right">% of subtotal</th></tr></thead><tbody id="qcSubcategoryBody"></tbody></table></div>
    <div class="card"><h3 class="section-title">Prevention Actions</h3><table><thead><tr><th>How to Avoid</th><th class="right">Orders</th><th class="right">% of subtotal</th></tr></thead><tbody id="qcPreventionBody"></tbody></table></div>
  </section>
  ...
  <section id="factory-share-page" class="page">
    ...
    <div class="card"><h3 class="section-title">Qarma Errors</h3><div style="overflow:auto"><table><thead><tr><th>Order</th><th>Actual Shipping Date</th><th>Qarma Rejected</th><th>Reinspected</th><th class="right">Reinspection Count</th><th>Qarma Comment</th><th>Qarma Report</th></tr></thead><tbody id="factoryShareQarmaErrors"></tbody></table></div></div>
    ...
    <div class="card"><h3 class="section-title" id="factoryShareSelectedTitle">Selected Orders — All completed orders in period</h3><div class="factory-share-orders-scroll"><table><thead><tr><th>Order</th><th>Customer Ref</th><th>Created Date</th><th>Actual Shipping Date</th><th>Customer</th><th class="right">Order QTY</th><th>Qarma Checked</th><th>Qarma Rejected</th><th>Reinspected</th><th class="right">Reinspection Count</th><th>Qarma Comment</th><th>Qarma Report</th><th>Remake</th></tr></thead><tbody id="factoryShareOrders"></tbody></table></div></div>
...
function normalizeRemakeCulprit(value) {{
  const culprit=String(value||'').trim();
  if (!culprit) return 'Unassigned';
  return culprit.toUpperCase()==='OA' ? 'Custimoo' : culprit;
}}
function remakeRowMatchesSelections(r) {{
  const category=String(r.category||'').trim()||'Uncategorized';
  const culprit=normalizeRemakeCulprit(r.culprit);
  if (category.toUpperCase()==='NOT A REMAKE') return false;
...
function syncCulpritFilters() {{
  const values=[...new Set(['Factory','Customer','Merchant','Shipping courier','Custimoo','Unassigned'].concat(remakeFilterRows().map(function(r) {{ return normalizeRemakeCulprit(r.culprit); }}).filter(Boolean)))].sort();
  ['summaryCulpritChoices','ytdCulpritChoices'].forEach(function(id) {{ const el=document.getElementById(id); if (!el) return; el.innerHTML=''; [['','All culprits']].concat(values.map(function(v) {{ return [v,v]; }})).forEach(function(pair) {{ const label=document.createElement('label'); const input=document.createElement('input'); input.type='checkbox'; input.value=pair[0]; input.checked=visibleRemakeCulprits.size===0 || visibleRemakeCulprits.has(pair[0]); input.addEventListener('change', function() {{ const wasAll=visibleRemakeCulprits.size===0; const selected=new Set(Array.from(el.querySelectorAll('input:checked')).map(function(other) {{ return other.value; }}).filter(Boolean)); if (pair[0]==='') visibleRemakeCulprits=new Set(); else if (wasAll && input.checked) visibleRemakeCulprits=new Set([pair[0]]); else visibleRemakeCulprits=selected; document.querySelectorAll('#summaryCulpritChoices input,#ytdCulpritChoices input').forEach(function(other) {{ other.checked=visibleRemakeCulprits.size===0 || visibleRemakeCulprits.has(other.value); }}); applyCategorySelection(visibleRemakeCategories); }}); label.appendChild(input); label.appendChild(document.createTextNode(' '+pair[1])); el.appendChild(label); }}); }});
}}
...
function selectedCategoriesFromControls(menu) {{ const selected=new Set(Array.from(menu.querySelectorAll('input:checked')).map(function(i) {{ return i.value; }})); return selected.size ? selected : new Set(['__NONE__']); }}
function updateFilterAllButtons() {{ document.querySelectorAll('.filter-all-btn').forEach(function(btn) {{ const all=btn.dataset.filterGroup==='category' ? visibleRemakeCategories.size===0 : visibleRemakeCulprits.size===0; btn.textContent=all ? 'Deselect all' : 'Select all'; }}); }}
function toggleAllFilterOptions(group) {{ if (group==='category') visibleRemakeCategories=visibleRemakeCategories.size===0 ? new Set(['__NONE__']) : new Set(); else visibleRemakeCulprits=visibleRemakeCulprits.size===0 ? new Set(['__NONE__']) : new Set(); if (group==='category') applyCategorySelection(visibleRemakeCategories); else applyCulpritSelection(visibleRemakeCulprits); updateFilterAllButtons(); }}
function applyCulpritSelection(value) {{ visibleRemakeCulprits=value instanceof Set ? new Set(value) : new Set(value ? [value] : []); document.querySelectorAll('#summaryCulpritChoices input,#ytdCulpritChoices input').forEach(function(input) {{ input.checked=visibleRemakeCulprits.size===0 || visibleRemakeCulprits.has(input.value); }}); applyCategorySelection(visibleRemakeCategories); updateFilterAllButtons(); }}
function resetToDefaultFilters() {{
  visibleRemakeCulprits=new Set(); visibleRemakeCategories=new Set(); excludeForceMajourGlobal=false; remakeAnalysisExcludeForceMajour=false;
  ACTIVE_PERIOD='ytd';
  [['measureFilter','orders'],['periodFilter','ytd'],['breakdownFilter','factory'],['ytdFactoryFilter',''],['ytdMeasureFilter','orders'],['remakeAnalysisFactoryFilter',''],['remakeAnalysisPeriodFilter','ytd']].forEach(function(pair) {{ const el=document.getElementById(pair[0]); if (el) el.value=pair[1]; }});
  syncChoiceGroups(); syncCategoryFilters(); syncCulpritFilters(); applyCategorySelection(new Set());
  renderForensics('', 'ytd');
}}
function applyCategorySelection(set) {{ visibleRemakeCategories=new Set(set||[]); excludeForceMajourGlobal=visibleRemakeCategories.size>0 && !visibleRemakeCategories.has('Force Majour'); remakeAnalysisExcludeForceMajour=false; applyForceMajourToReport(); syncCategoryFilters(); updateSummaryStats(); applyPeriod(ACTIVE_PERIOD); ytdCumulativeTable(); renderYtdFactoryTable(); renderYtdChart(); renderDetails(); renderGroupingTable((document.getElementById('breakdownFilter')||{{value:'factory'}}).value); updateFilterAllButtons(); }}
...
function updateYtdKpis() {{
  ...
  const ytdAgg = ytdSummaryAggregates();
  if (YTD_MEASURE === 'orders') {{
    document.getElementById('ytdVolume').textContent = ytdAgg.totalOrders.toLocaleString();
    document.getElementById('periodKpiValue').textContent = ytdAgg.totalRemakeOrders.toLocaleString();
    document.getElementById('periodKpiSub').textContent = 'YTD · ' + (ytdAgg.totalOrders ? (ytdAgg.totalRemakeOrders/ytdAgg.totalOrders*100).toFixed(2) : '0.00') + '% of ' + ytdAgg.totalOrders.toLocaleString() + ' orders are remakes';
    document.querySelector('#ytdVolume').closest('.metric').querySelector('.label').textContent = 'Selected Period No of Orders';
  }} else {{
    document.getElementById('ytdVolume').textContent = ytdAgg.totalVolume.toLocaleString();
    document.getElementById('periodKpiValue').textContent = ytdAgg.totalRemakeQty.toLocaleString();
    document.getElementById('periodKpiSub').textContent = 'YTD · ' + (ytdAgg.totalVolume ? (ytdAgg.totalRemakeQty/ytdAgg.totalVolume*100).toFixed(2) : '0.00') + '% of ' + ytdAgg.totalVolume.toLocaleString() + ' items · ' + ytdAgg.totalRemakeQty.toLocaleString() + ' remake qty';
...
function ytdCumulativeTable() {{
  ...
  if (YTD_MEASURE === 'orders') {{
    ...
    body.innerHTML = ytdView.months.map(function(m, i) {{
      const rate = (ytdView.cumulativeOrders[i] || 0) > 0 ? remOrdersCum[i] / ytdView.cumulativeOrders[i] * 100 : 0;
      return '<tr><td><strong>' + m + '</strong></td><td class="right">' + (ytdView.monthlyOrders[i] || 0).toLocaleString() + '</td><td class="right">' + (ytdView.cumulativeOrders[i] || 0).toLocaleString() + '</td><td class="right">' + ((ytdView.monthlyRemakeOrders || [])[i] || 0).toLocaleString() + '</td><td class="right">' + remOrdersCum[i].toLocaleString() + '</td><td class="right">' + pctPill(rate) + '</td></tr>';
    }}).join('');
    ...
    body.innerHTML = ytdView.months.map(function(m, i) {{
      const rate = (ytdView.cumulativeVolume[i] || 0) > 0 ? remQtyCum[i] / ytdView.cumulativeVolume[i] * 100 : 0;
      return '<tr><td><strong>' + m + '</strong></td><td class="right">' + (ytdView.monthlyVolume[i] || 0).toLocaleString() + '</td><td class="right">' + (ytdView.cumulativeVolume[i] || 0).toLocaleString() + '</td><td class="right">' + ((ytdView.monthlyRemakeQty || [])[i] || 0).toLocaleString() + '</td><td class="right">' + remQtyCum[i].toLocaleString() + '</td><td class="right">' + pctPill(rate) + '</td></tr>';
    }}).join('');
...
syncCategoryFilters();
syncCulpritFilters();
document.querySelectorAll('.reset-default-btn').forEach(function(btn) {{ btn.addEventListener('click', resetToDefaultFilters); }});
document.querySelectorAll('.filter-all-btn').forEach(function(btn) {{ btn.addEventListener('click', function() {{ toggleAllFilterOptions(btn.dataset.filterGroup); }}); }});
updateFilterAllButtons();
...
var remakeAnalysisResponsibilityFilter = '';
var remakeAnalysisExcludeForceMajour = false;
var currentFilteredRemakeRows = [];
function renderForensics(factoryFilter, periodKey) {{
  ...
  shareLists.forEach(function(list) {{ list.forEach(function(r) {{ const orderKey=String(r.order||'').replace(/^#/,'').trim(); const rowMonth=String(r.actual_shipping_date||r.completed_date||r.created_date||'').slice(0,7); if (REMAKE_ORDER_NUMBERS.has(orderKey) && ((periodKey || 'ytd') === 'all' || (rowMonth && periodMonths.has(rowMonth)))) authoritativeRemakes.add(orderKey); }}); }});
  ...
  function culpritResponsibility(value) {{
    const culprit = normalizeRemakeCulprit(value);
    if (culprit === 'Unassigned') return 'Unassigned';
    if (culprit === 'Custimoo') return 'Custimoo';
    return 'Factory';
  }}
  const filteredRemakes = factoryRemakes.filter(function(r) {{
    const category = String(r.category || '').trim() || 'Uncategorized';
    const culprit = normalizeRemakeCulprit(r.culprit);
    return remakeRowMatchesSelections(r) && (!remakeAnalysisCategoryFilter || category === remakeAnalysisCategoryFilter) && (!remakeAnalysisCulpritFilter || culprit === remakeAnalysisCulpritFilter) && (!remakeAnalysisResponsibilityFilter || culpritResponsibility(culprit) === remakeAnalysisResponsibilityFilter);
  }});
  ...
  const remCat = {{}}, remCul = {{}};
  filteredRemakes.forEach(function(r) {{ const c=String(r.category||'Uncategorized').trim()||'Uncategorized', u=normalizeRemakeCulprit(r.culprit), sub=(u === 'Custimoo' ? (String(r.culprit_subcategory||'').trim() || 'Unassigned') : '—'); const catGroup=remCat[c]||(remCat[c]={{orders:0,qty:0}}); catGroup.orders++; catGroup.qty += Number(r.qty)||0; const culpritKey=u+'||'+sub; const culpritGroup=remCul[culpritKey]||(remCul[culpritKey]={{orders:0,qty:0,culprit:u,subcategory:sub}}); culpritGroup.orders++; culpritGroup.qty += Number(r.qty)||0; }});
  ...
  const reviewedSubtotal = remReviewed.reduce(function(sum,r) {{ return sum+Number(r.qty||0); }} ,0);
  ...
  const largeUnOrderSubtotal = largeUn.length;
  document.getElementById('remakeReviewedBody').innerHTML = remReviewed.slice().sort(function(a,b) {{ return Number(b.qty||0)-Number(a.qty||0); }}).map(function(r) {{ return '<tr><td>#'+esc(remakeOrderKey(r))+'</td><td>'+esc(r.customer||'(unknown)')+'</td><td class="right">'+(Number(r.qty)||0).toLocaleString()+'</td><td class="right">'+(reviewedSubtotal ? (Number(r.qty||0)/reviewedSubtotal*100).toFixed(1)+'%' : '0.0%')+'</td><td>'+esc(r.factory||'')+'</td><td>'+esc(r.category||'—')+'</td><td>'+esc(normalizeRemakeCulprit(r.culprit)||'—')+'</td><td>'+esc(r.culprit_subcategory||'—')+'</td><td>'+esc(r.verification_status||'—')+'</td><td>'+esc(r.comment||'—')+'</td></tr>'; }}).join('') || '<tr><td colspan="10">No reviewed remake orders match this factory.</td></tr>';
  document.getElementById('remakeLargeUnreviewedBody').innerHTML = largeUn.map(function(r) {{ return '<tr><td>#'+esc(remakeOrderKey(r))+'</td><td>'+esc(r.customer||'(unknown)')+'</td><td class="right"><strong>'+(Number(r.qty)||0).toLocaleString()+'</strong></td><td class="right">'+(largeUnOrderSubtotal ? (1/largeUnOrderSubtotal*100).toFixed(1)+'%' : '0.0%')+'</td><td class="right">'+(largeUnSubtotal ? (Number(r.qty||0)/largeUnSubtotal*100).toFixed(1)+'%' : '0.0%')+'</td><td>'+esc(r.factory||'')+'</td><td>'+esc(r.admin||'')+'</td><td>'+esc(r.month||'')+'</td></tr>'; }}).join('') || '<tr><td colspan="7">No unreviewed remake orders match this factory.</td></tr>';
  function groupHtml(g, kind) {{
    if (!kind) {{
      const subtotal=Object.keys(g).reduce(function(sum,k) {{ return sum + Number(g[k].qty||0); }},0);
      const orderTotal=Object.keys(g).reduce(function(sum,k) {{ return sum + Number(g[k].orders||0); }},0);
      return Object.keys(g).sort(function(a,b) {{ return (g[b].qty||0)-(g[a].qty||0) || a.localeCompare(b); }}).map(function(k) {{ const item=g[k], share=subtotal ? (item.qty/subtotal*100).toFixed(1)+'%' : '0.0%'; return '<tr><td>'+esc(k)+'</td><td class="right">'+item.orders.toLocaleString()+'</td><td class="right">'+item.qty.toLocaleString()+'</td><td class="right">'+share+'</td></tr>'; }}).join('');
    }}
    const qtySubtotal=Object.keys(g).reduce(function(sum,k) {{ return sum + Number(g[k].qty||0); }},0); const orderSubtotal=Object.keys(g).reduce(function(sum,k) {{ return sum + Number(g[k].orders||0); }},0); return Object.keys(g).sort(function(a,b) {{ return (g[b].qty||0)-(g[a].qty||0) || a.localeCompare(b); }}).map(function(k) {{ const item=g[k], orderShare=orderSubtotal ? (item.orders/orderSubtotal*100).toFixed(1)+'%' : '0.0%', qtyShare=qtySubtotal ? (item.qty/qtySubtotal*100).toFixed(1)+'%' : '0.0%', filterValue=kind === 'culprit' ? item.culprit : k, active=(kind === 'category' ? remakeAnalysisCategoryFilter : remakeAnalysisCulpritFilter) === filterValue; return '<tr class="remake-filter-row'+(active ? ' active' : '')+'" data-filter-kind="'+kind+'" data-filter-value="'+escapeAttr(filterValue)+'" tabindex="0" role="button" aria-pressed="'+(active ? 'true' : 'false')+'"><td>'+esc(kind === 'culprit' ? item.culprit : k)+'</td>'+(kind === 'culprit' ? '<td>'+esc(item.subcategory||'—')+'</td>' : '')+'<td class="right">'+item.orders.toLocaleString()+'</td><td class="right">'+orderShare+'</td><td class="right">'+item.qty.toLocaleString()+'</td><td class="right">'+qtyShare+'</td></tr>'; }}).join('') + '<tr class="total-row"><td'+(kind === 'category' ? '' : ' colspan="2"')+'><strong>Subtotal</strong></td><td class="right"><strong>'+orderSubtotal.toLocaleString()+'</strong></td><td class="right"><strong>100.0%</strong></td><td class="right"><strong>'+qtySubtotal.toLocaleString()+'</strong></td><td class="right"><strong>100.0%</strong></td></tr>'; }}
  ...
  currentFilteredRemakeRows = filteredRemakes.slice();
  document.getElementById('remakeFilteredOrdersBody').innerHTML = filteredRemakes.slice().sort(function(a,b) {{ return Number(b.qty||0)-Number(a.qty||0) || remakeOrderKey(a).localeCompare(remakeOrderKey(b)); }}).map(function(r) {{ return '<tr><td>#'+esc(remakeOrderKey(r))+'</td><td>'+esc(r.customer||'(unknown)')+'</td><td class="right">'+(Number(r.qty)||0).toLocaleString()+'</td><td>'+esc(r.factory||'')+'</td><td>'+esc(r.category||'Uncategorized')+'</td><td>'+esc(r.culprit||'Unassigned')+'</td><td>'+esc(r.culprit_subcategory||'—')+'</td><td>'+esc(r.verification_status||'—')+'</td><td>'+esc(r.comment||'—')+'</td></tr>'; }}).join('') || '<tr><td colspan="9">No remake orders match the selected filters.</td></tr>';
  const wrongTypeRows=remakeFilterRows().filter(function(r) {{ return String(r.category||'').trim().toUpperCase()==='NOT A REMAKE'; }}).filter(function(r) {{ const f=String(r.factory||'').split(',').map(function(x) {{ return x.trim(); }}); const m=String(r.month||r.created_date||r.date||'').slice(0,7); return (!factoryFilter || f.indexOf(factoryFilter)>=0) && ((periodKey||'ytd')==='all' || !m || periodMonths.has(m)); }});
  document.getElementById('remakeWrongTypeBody').innerHTML=wrongTypeRows.slice().sort(function(a,b) {{ return Number(b.qty||0)-Number(a.qty||0) || remakeOrderKey(a).localeCompare(remakeOrderKey(b)); }}).map(function(r) {{ return '<tr><td>#'+esc(remakeOrderKey(r))+'</td><td>'+esc(r.customer||'(unknown)')+'</td><td class="right">'+(Number(r.qty)||0).toLocaleString()+'</td><td>'+esc(r.factory||'')+'</td><td>'+esc(r.month||r.created_date||r.date||'')+'</td><td>'+esc(r.comment||'—')+'</td></tr>'; }}).join('') || '<tr><td colspan="6">No orders categorized NOT A REMAKE match this factory and period.</td></tr>';
  ...
  const qcUnreviewedSubtotal = qcUn.reduce(function(sum,r) {{ return sum+Number(r.total_qty||r.defects_qty||0); }},0);
  document.getElementById('qcUnreviewedBody').innerHTML = qcUn.slice().sort(function(a,b) {{ return Number(b.total_qty||b.defects_qty||0)-Number(a.total_qty||a.defects_qty||0); }}).map(function(r) {{ return '<tr><td>#'+esc(qcRejectionKey(r))+'</td><td>'+esc(r.factory||'')+'</td><td class="right">'+(Number(r.total_qty)||0).toLocaleString()+'</td><td class="right">'+(qcUnreviewedSubtotal ? (Number(r.total_qty||r.defects_qty||0)/qcUnreviewedSubtotal*100).toFixed(1)+'%' : '0.0%')+'</td><td class="right">'+(Number(r.defects_qty)||0).toLocaleString()+'</td><td>'+esc(r.date||'')+'</td></tr>'; }}).join('') || '<tr><td colspan="5">All QC rejections have forensic notes.</td></tr>';
  document.getElementById('qcErrorTypeBody').innerHTML = groupHtml(et); document.getElementById('qcSubcategoryBody').innerHTML = groupHtml(escat); const preventionSubtotal=Object.keys(pa).reduce(function(sum,k) {{ return sum+pa[k]; }},0); document.getElementById('qcPreventionBody').innerHTML = Object.keys(pa).sort(function(a,b) {{ return pa[b]-pa[a] || a.localeCompare(b); }}).map(function(k) {{ return '<tr><td>'+esc(k)+'</td><td class="right">'+pa[k].toLocaleString()+'</td><td class="right">'+(preventionSubtotal ? (pa[k]/preventionSubtotal*100).toFixed(1)+'%' : '0.0%')+'</td></tr>'; }}).join('');
}}
function exportFilteredRemakesCsv() {{
  const headers=['Order','Customer','QTY','Factory','Category','Culprit','Subcategory','Verification','Comment'];
  const csvCell=function(v) {{ const s=String(v == null ? '' : v); return '"'+s.replace(/"/g,'""')+'"'; }};
  const lines=[headers].concat(currentFilteredRemakeRows.map(function(r) {{ return [remakeOrderKey(r),r.customer||'',r.qty||0,r.factory||'',r.category||'Uncategorized',r.culprit||'Unassigned',r.culprit_subcategory||'—',r.verification_status||'—',r.comment||''].map(csvCell); }})).map(function(row) {{ return row.join(','); }});
  const blob=new Blob([lines.join('\\r\\n')], {{type:'text/csv;charset=utf-8;'}}), url=URL.createObjectURL(blob), a=document.createElement('a'); a.href=url; a.download='filtered-remake-orders.csv'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
}}
...
(function() {{
  ...
  const exportCsv=document.getElementById('exportFilteredRemakesCsv');
  if (exportCsv) exportCsv.addEventListener('click', exportFilteredRemakesCsv);
  const periodSelect=document.getElementById('remakeAnalysisPeriodFilter');
  const responsibilitySelect=document.getElementById('remakeAnalysisResponsibilityFilter');
  const excludeForceMajour=document.getElementById('remakeAnalysisExcludeForceMajour');
  if (excludeForceMajour) excludeForceMajour.addEventListener('change', function() {{ setForceMajourGlobal(excludeForceMajour.checked); }});
  ...
  if (responsibilitySelect) responsibilitySelect.addEventListener('change', function() {{ remakeAnalysisResponsibilityFilter = responsibilitySelect.value; rerender(); }});
...
function showFactorySharePage(slug) {{
  ...
  document.getElementById('factoryShareKpis').innerHTML = [['Total Orders',periodFactory.orders||0,'all'],['Total Order QTY',periodFactory.volume||0,'all'],['Remake Orders',periodFactory.remake_orders||0,'remake'],['Remake QTY',periodFactory.remake_qty||0,'remake'],['Qarma Orders Checked',periodQ.orders_checked||0,'qarma_checked'],['Qarma Error Orders',periodQ.rejected_orders||0,'qarma_rejected'],['Orders with Reinspection',periodQ.orders_with_reinspection||0,'reinspection'],['Reinspection Events',0,'reinspection_events']].map(function(x) {{ return '<div class="card metric factory-filter-card" data-filter="'+x[2]+'" title="Click to show matching orders" style="cursor:pointer"><div class="label">'+x[0]+'</div><div class="value">'+Number(x[1]||0).toLocaleString()+'</div></div>'; }}).join('');
  const periodMonths = new Set(period.monthKeys || MONTH_KEYS);
  ...
  const rejectedCard=document.querySelector('.factory-filter-card[data-filter="qarma_rejected"]'); if (rejectedCard) rejectedCard.querySelector('.value').textContent=orders.filter(function(r) {{ return r.qarma_rejected_period; }}).length.toLocaleString();
  ...
  function renderRemakeSelectedRows(list) {{
    const table=document.getElementById('factoryShareOrders').closest('table');
    table.querySelector('thead').innerHTML='<tr><th>Order</th><th>Customer Ref</th><th>Created Date</th><th>Actual Shipping Date</th><th>Customer</th><th>Order QTY</th><th>Category</th><th>Culprit</th><th>Subcategory</th><th>Verification</th><th>Comment</th></tr>';
    const byOrder=new Map((typeof REMAKES!=='undefined'?REMAKES:[]).map(function(r) {{ return [String(r.order||'').replace(/^#/,'').trim(),r]; }}));
    document.getElementById('factoryShareOrders').innerHTML=list.map(function(r) {{ const order=String(r.order||'').replace(/^#/,'').trim(); const a=byOrder.get(order)||{{}}; return '<tr data-order="'+escapeAttr(order)+'"><td>#'+esc(order)+'</td><td>'+esc(r.customer_ref||'—')+'</td><td>'+dateOnly(r.created_date)+'</td><td>'+dateOnly(r.actual_shipping_date)+'</td><td>'+esc(r.customer||'')+'</td><td class="right">'+Number(r.qty||0).toLocaleString()+'</td><td><select class="remake-edit remake-category" aria-label="Category for order '+escapeAttr(order)+'">'+remakeCategoryOptions(a.category||'')+'</select></td><td><select class="remake-edit remake-culprit" aria-label="Culprit for order '+escapeAttr(order)+'">'+remakeCulpritOptions(a.culprit||'')+'</select></td><td>'+renderCustimooSubcategory(a, order)+'</td><td>'+esc(a.verification_status||'—')+'</td><td><textarea class="remake-edit remake-comment" aria-label="Comment for order '+escapeAttr(order)+'" rows="3" placeholder="Comment">'+esc(a.comment||'')+'</textarea></td></tr>'; }}).join('') || '<tr><td colspan="11">No remake orders in the selected period.</td></tr>';
  }}
  function restoreStandardOrderHeader() {{ document.getElementById('factoryShareOrders').closest('table').querySelector('thead').innerHTML='<tr><th>Order</th><th>Customer Ref</th><th>Created Date</th><th>Actual Shipping Date</th><th>Customer</th><th class="right">Order QTY</th><th>Qarma Checked</th><th>Qarma Rejected</th><th>Reinspected</th><th class="right">Reinspection Count</th><th>Qarma Comment</th><th>Qarma Report</th><th>Remake</th></tr>'; }}
  function renderFactoryAnalysis(selectedList) {{
    selectedList = selectedList || orders;
    const selectedOrderSet = new Set(selectedList.map(function(r) {{ return String(r.order||''); }}));
    const qErrors=selectedList.filter(function(r) {{ return r.qarma_rejected_period; }});
    document.getElementById('factoryShareQarmaErrors').innerHTML=qErrors.map(function(r) {{ const link=/^https?:\\/\\//i.test(r.qarma_report_link||'') ? '<a href="'+escapeAttr(r.qarma_report_link)+'" target="_blank" rel="noopener">Open report</a>' : '—'; return '<tr><td>#'+esc(r.order)+'</td><td>'+dateOnly(r.actual_shipping_date)+'</td><td>Yes</td><td>'+ (r.qarma_reinspected_factory?'Yes':'No') +'</td><td class="right">'+Number(r.qarma_reinspection_count_factory||0).toLocaleString()+'</td><td>'+esc(r.qarma_comment||'—')+'</td><td>'+link+'</td></tr>'; }}).join('') || '<tr><td colspan="7">No Qarma errors for this factory and period.</td></tr>';
    ...
    const rem=(typeof REMAKES!=='undefined'?REMAKES:[]).filter(function(r) {{ return matchPeriod(r) && selectedOrderSet.has(String(r.order||'')); }});
    document.getElementById('factoryShareRemakeAnalysis').innerHTML=rem.map(function(r) {{ return '<tr><td>#'+esc(r.order||'')+'</td><td>'+Number(r.qty||r.total_qty||0).toLocaleString()+'</td><td>'+esc(r.category||'—')+'</td><td>'+esc(normalizeRemakeCulprit(r.culprit)||'—')+'</td><td>'+esc(r.culprit_subcategory||'—')+'</td><td>'+esc(r.source||'—')+'</td><td>'+esc(r.verification_status||'—')+'</td><td>'+esc(r.comment||'—')+'</td></tr>'; }}).join('') || '<tr><td colspan="8">No remake analysis rows for this factory and period.</td></tr>';
    const qc=(typeof QC_REJECTIONS!=='undefined'?QC_REJECTIONS:[]).filter(function(r) {{ return matchFactory(r) && matchPeriod(r) && selectedOrderSet.has(String(r.order||'')); }});
  ...
  document.querySelectorAll('.factory-filter-card').forEach(function(card) {{ card.addEventListener('click', function() {{ const filter=card.dataset.filter; const subset=filter==='all' ? orders : filter==='remake' ? orders.filter(function(r) {{ return r.remake; }}) : filter==='qarma_checked' ? orders.filter(function(r) {{ return r.qarma_checked_period; }}) : filter==='qarma_rejected' ? orders.filter(function(r) {{ return r.qarma_rejected_period; }}) : filter==='reinspection' ? orders.filter(function(r) {{ return r.qarma_reinspected_factory; }}) : filter==='reinspection_events' ? orders.filter(function(r) {{ return Number(r.qarma_reinspection_count_factory||0) > 0; }}) : orders.filter(function(r) {{ return r.qarma_reinspected_factory; }}); const labels={{all:'All completed orders',remake:'Remake orders',qarma_checked:'Qarma checked orders',qarma_rejected:'Qarma rejected orders',reinspection:'Orders with reinspection',reinspection_events:'Reinspection events'}}; document.getElementById('factoryShareSelectedTitle').textContent='Selected Orders — '+labels[filter]+' ('+subset.length+')'; if (filter==='remake') renderRemakeSelectedRows(subset); else {{ restoreStandardOrderHeader(); renderFactoryOrderRows(subset); }} renderFactoryMonthly(subset); renderFactoryAnalysis(subset); document.getElementById('factoryShareSelectedTitle').scrollIntoView({{behavior:'smooth',block:'start'}}); }}); }});
  document.getElementById('factoryShareOrders').addEventListener('input', function(ev) {{
    const input=ev.target; if (!input.classList.contains('remake-edit')) return;
    const row=remakeRows.find(function(r) {{ return remakeOrderKey(r)===input.closest('tr').dataset.order; }}); if (!row) return;
    if (input.classList.contains('remake-comment')) row.comment=input.value;
    remakeDirtyFields.set(remakeOrderKey(row), new Set([...(remakeDirtyFields.get(remakeOrderKey(row)) || []), 'comment'])); scheduleRemakeSave();
  }});
  document.getElementById('factoryShareOrders').addEventListener('change', function(ev) {{
    const input=ev.target; if (!input.classList.contains('remake-edit')) return;
    const row=remakeRows.find(function(r) {{ return remakeOrderKey(r)===input.closest('tr').dataset.order; }}); if (!row) return;
    if (input.classList.contains('remake-category')) {{ row.category=input.value; remakeDirtyFields.set(remakeOrderKey(row), new Set([...(remakeDirtyFields.get(remakeOrderKey(row)) || []), 'category'])); }}
    if (input.classList.contains('remake-culprit')) {{ row.culprit=input.value; if (row.culprit !== 'Custimoo') row.culprit_subcategory=''; remakeDirtyFields.set(remakeOrderKey(row), new Set([...(remakeDirtyFields.get(remakeOrderKey(row)) || []), 'culprit', 'culprit_subcategory'])); updateCustimooSubcategoryCell(row, remakeOrderKey(row), input); }}
